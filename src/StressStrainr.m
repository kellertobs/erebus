...
MP.Edot(:,3)  =  1/2.*(Edot(:,3) + Edot(:,4)) + 1e-32;
...
MP.Tau        =  MP.EtaVEP.*MP.Edot + MP.Xi.*MP.Taur;
...
MP.Diss       = (MP.Edot(:,1) - EdotE(:,1)).*MP.Tau(:,1) + ...
                (MP.Edot(:,2) - EdotE(:,2)).*MP.Tau(:,2) + ...
             2.*(MP.Edot(:,3) - EdotE(:,3)).*MP.Tau(:,3);
MP.Diss       = max(0,MP.Diss);
