...
% *************************************************************************
% 
% LACO      Finite-element Stokes flow solver with visco-elastic/brittle-
...
while CTX.TIME.total < CTX.TIME.end
    ...
    CTX.TIME.record = [CTX.TIME.record,CTX.TIME.total];
    ReportTiming(CTX);
...
%**************************************************************************
    ...
    if  mod(CTX.TIME.istep,CTX.IO.nwrite) == 0
        ...
        PlotOutput(CTX,CTX.IO.frame,1,CTX.IO.frame,'W','srf_rr','flip','print');
