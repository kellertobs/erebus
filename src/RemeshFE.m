...
if apply_topx == 1
    d            = Coord(1,:,1) - Xo(FE.MapQ2(1,:),1)';
    Coord(:,:,1) = Coord(:,:,1) - repmat(d,FE.nzQ2,1) .* repmat(linspace(1,0,FE.nzQ2).',1,FE.nxQ2).^n;
end

if apply_topz == 1
    d            = Coord(1,:,2) - Xo(FE.MapQ2(1,:),2)';
    Coord(:,:,2) = Coord(:,:,2) - repmat(d,FE.nzQ2,1) .* repmat(linspace(1,0,FE.nzQ2).',1,FE.nxQ2).^n;
end

% upper boundary topography
if apply_botx == 1
    d            = Coord(FE.nzQ2,:,1) - Xo(FE.MapQ2(FE.nzQ2,:),1)';
    Coord(:,:,1) = Coord(:,:,1) - repmat(d,FE.nzQ2,1) .* repmat(linspace(1,0,FE.nzQ2).',1,FE.nxQ2).^n;
end

if apply_botz == 1
    d            = Coord(FE.nzQ2,:,2) - Xo(FE.MapQ2(FE.nzQ2,:),2)';
    Coord(:,:,2) = Coord(:,:,2) - repmat(d,FE.nzQ2,1) .* repmat(linspace(1,0,FE.nzQ2).',1,FE.nxQ2).^n;
end
...
if apply_leftx == 1
    d            = Coord(:,1,1) - Xo(FE.MapQ2(:,1),1);
    Coord(:,:,1) = Coord(:,:,1) - repmat(d,1,FE.nxQ2) .* min(1,max(0,((FE.W-Coord(:,:,1))./FE.W))).^n;
end

if apply_leftz == 1
    d            = Coord(:,1,2) - Xo(FE.MapQ2(:,1),2);
    Coord(:,:,2) = Coord(:,:,2) - repmat(d,1,FE.nxQ2) .* min(1,max(0,((FE.W-Coord(:,:,1))./FE.W))).^n;
end

% right boundary topography
if apply_rightx == 1
    d            = Coord(:,FE.nxQ2,1) - Xo(FE.MapQ2(:,FE.nxQ2),1);
    Coord(:,:,1) = Coord(:,:,1) - repmat(d,1,FE.nxQ2) .*  min(1,max(0,(Coord(:,:,1)./FE.W))).^n;
end

if apply_rightz == 1
    d            = Coord(:,FE.nxQ2,2) - Xo(FE.MapQ2(:,FE.nxQ2),2);
    Coord(:,:,2) = Coord(:,:,2) - repmat(d,1,FE.nxQ2) .*  min(1,max(0,(Coord(:,:,1)./FE.W))).^n;
end
